...
typedef struct sockfd {
	int	inuse;		// Are we using it?
        ...
        int     closed;         // VP : Indicate that the other end has closed, but there 
                                // still may be some data in the buffers to read
...
static void sock_close(int fd) {
	...
	// VP : this was missing :)
	if (fds[fd].recv_buf) {
	  pbuf_free(fds[fd].recv_buf);
	  fds[fd].recv_buf = NULL;
	}
}
...
int lwip_connect(int s, struct sockaddr *name, socklen_t namelen) {
	...
	if (fd->type == SOCK_STREAM) {
		...
		while (fd->connerr > 0) {
		        /* VP : added a timeout */
			cond_wait_timed(fd->connect, fd->mutex, 4000);
			if (fd->connerr > 0) {
			  printf("connect : connerr = %d\n", fd->connerr);
			  if (tcp_close(fd->tcppcb) != ERR_OK)
			    tcp_abort(fd->tcppcb);
			  fd->tcppcb = NULL;
			  errno = EINVAL; // TODO : set a correct errno 
			                  // (keep the one of cond_wait_timed ?)
			  rv = -1; goto out;
			}
		}
...
static int recv_common(int s, void * mem, int len, unsigned int flags, int istcp) {
	...
	// VP : if the other end is closed and the buffer empty, then it's over
	// TODO : set errno
	if (fd->closed && fd->recv <= 0) {
	  fd->recv = -1; rv = -1; goto out;
	}
	...
	while (fd->recv <= 0) {
		...
		/* VP : added a timeout (10 seconds) */
		res = cond_wait_timed(fd->recv_avail, fd->mutex, 10000);
		if (res && fd->recv <= 0) {
		        if (errno != EAGAIN) {
			  printf("recv_common : cond_wait_timeout returned with error "
				 "but errno is %d (!= EAGAIN)\n", errno);
			}
		        printf("recv_common : timeout (cond_wait_timeout returned %d)\n", 
			       res);
			fd->recv = -1; rv = 0; goto out;
		}
