...
void thd_schedule_next(kthread_t *thd) {
	/* Can't boost a blocked thread */
        /* VP : ??? a bug ? why should we check thd_current here ? 
	   mean thd instead ? */
        /* OK , I changed thd_current to thd and hope for the best,
	   also added the test thd==thd_current */
	if (thd_current == thd || thd->state != STATE_READY)
		return;

	/* just wondering ... can it happen ?? */
	assert(thd_current);
